...
casper.start('http://paygo.crawlera.com:8010/fetch?url=http://wtfismyip.com/text', function(response) {
    this.echo(response.status);  // print response status
});
// or
//  2. call url through authentication and crawlera directly for each url
casper.start('http://<API key>:@paygo.crawlera.com:8010/fetch?url=http://wtfismyip.com/text', function(response) {
    this.echo(response.status);  // print response status
});
