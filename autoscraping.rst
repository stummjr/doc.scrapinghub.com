...
:doc:`cloud` for details) with the scrapy extensions, middlewares and settings written for your specific needs. As inside the same scrapy
...
Extending the autoscraping bot - II. Spiderlets
===============================================

A set of general extensions for the entire autoscraping project may not be enough. Sometimes we need to write custom code for a specific
spider. There is a special addon that allows you to do that: the Spiderlets addon. In order to enable, you have to add it to your
project (see :doc:`addons`) and then add the setting
``SPIDERLETS_MODULE`` to the project settings, with the value being the module name that contains the spiderlets module. For example,
if your spiderlets are in the module ``myproject.spiderlets`` where ``myproject`` is the scrapy project package deployed in your panel
project, then you set the value of ``SPIDERLETS_MODULE`` to ``myproject.spiderlets``. Spiderlets module don't need to be inside a
scrapy project package, though. You can also deploy a common python package with it using the Panel egg deploy interface (see :ref:`Deploying eggs <deploy-egg>`)
...
the ``parse`` method of the autoscraping spider (see `Slybot Spider`_ ). Whenever you need to generate a FormRequest for sending post
...
input method, and it is applied only to those responses which its callback is the ``parse_login_page`` method of the autoscraping spider.
If you define ``parse_login_page`` method in your spiderlet, the request callback will be overriden by this new one.
...
.. _Slybot Spider: https://github.com/scrapy/slybot/blob/master/slybot/spider.py
